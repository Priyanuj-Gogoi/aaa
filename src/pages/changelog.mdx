...
- Fixed a issue with mentions not working
...
- Fixed `$roleID[]` and `$channelID[]` no longer stop code execution
...
- Fixed `$onlyPerms[]`
- Fixed a bug with `$` command prefix and `$message`
...
### 2.2.2
- Development slash commands and bug fixes
- Added development time slash command which are updated instantly on selected guilds
- Fixed bug within an app notification bar being stuck behind status bar
- Added automatic sync of gateway intents when a user comes back from developer portal 
- Added new text tutorial with translations
- Added notifications when a new update is available with ability to install it in the background
- Added integrated bug reporting
- Fixed inconsistent app bar positions
...
### 2.0.6
- Updated UI
- Added search bar in variables tab
- `BDScript 2` is now the default script language for new commands
- Added `Open wiki` button in functions list tab
- Removed stop hosting button
